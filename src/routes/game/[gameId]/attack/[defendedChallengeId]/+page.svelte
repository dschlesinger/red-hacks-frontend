  ...
  {:else if target}
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="col-span-1 border border-white/10 bg-slate-900/50 backdrop-blur-md rounded-xl p-5 space-y-4 h-fit">
        ...
        <p class="text-[10px] text-gray-600 mt-1">Resets the LLM conversation only.</p>
      </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between flex-wrap gap-2">
            ...
            <div class="px-3 py-1.5 rounded-lg bg-amber-900/30 border border-amber-500/30 text-xs font-mono text-amber-200">
              {#if (target?.challenges?.attack_steal_coins ?? 0) === 0}
                Next attempt reward: <strong>0</strong> coins (no steal configured)
              {:else}
                Next reward: <strong>{target?.challenges?.attack_steal_coins ?? 0}</strong> coins
              {/if}
            </div>
          </div>
          <p class="text-[11px] text-gray-500 font-mono">Final reward is determined by the server on success.</p>
